...
names = ['Cuba I', 'San Felipe II Okeechobee', 'Bahamas',
            'Cuba II', 'CubaBrownsville', 'Tampico', 'Labor Day',
             ...
             'Beulah', 'Camille', 'Edith', 'Anita', 'David', 'Allen',
              'Gilbert', 'Hugo', 'Andrew', 'Mitch', 'Isabel', 'Ivan',
               'Emily', 'Katrina', 'Rita', 'Wilma', 'Dean', 'Felix',
               'Matthew', 'Irma', 'Maria', 'Michael']

damages = ['Damages not recorded', '100M', 'Damages not recorded',
            '40M', '27.9M', '5M', 'Damages not recorded', '306M', '2M',
            '65.8M', '326M', '60.3M', '208M', '1.42B', '25.4M', 'Damages not recorded',
            '1.54B', '1.24B', '7.1B', '10B', '26.5B', '6.2B', '5.37B', '23.3B', '1.01B', '125B',
...
months = ['October', 'September', 'September', 'November', 'August', 'September', 'September',
            ...
            'October', 'September', 'September', 'July', 'August', 'September', 'October', 'August',
...
#damage modify. Changes content into floats, and appends them
#to a new list
def update_list(lst = damages):
    updated_damage_list = []
    for damage in damages:
        if damage == 'Damages not recorded':
            updated_damage_list.append(damage)
        elif "M" in damage:
            updated_damage_list.append(float(damage[:-1]) * 1000000)
        elif "B" in damage:
            updated_damage_list.append(float(damage[:-1]) * 1000000000)
...
def menue():
    for index, name in enumerate(names):
        print(f"\"{name}\" hurricane. Index number to enter for more information {index}")
    user_input = input("please choose your hurrican, enter the index number")
    return int(user_input)




def information_finder(user_input):

    hurricane_dictionary = {
                                "name": names[user_input],
                                "months": months[user_input],
                                "years": years[user_input],
                                "max_sustained_winds": max_sustained_winds[user_input],
                                "areas_affected": areas_affected[user_input],
                                "deaths":deaths[user_input]
                                }
    return hurricane_dictionary

#Calling function of the module here
#menue function, to pick your hurricane
user_input = menue()
print(information_finder(user_input))
